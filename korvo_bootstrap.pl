...
sub update_file {
    ...
    if($has_lwp) {
      ...
      $ua->env_proxy();     
      $response = $ua->get("http://anon:anon\@svn.research.cc.gatech.edu/kaos/korvo_base/trunk/$base");
	...
	return;
      ...
      my $ret = system("cd /tmp ; rm -f $base ; wget -q http://anon:anon\@svn.research.cc.gatech.edu/kaos/korvo_base/trunk/$base");
      my $exit_value = $ret >> 8;
      if ($exit_value != 0) {
	  if (!$suppress_errors) {
